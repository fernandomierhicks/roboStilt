...
class robot_state:
    ...
    def __init__(self):        
        ...
        rospy.init_node('robostilt_state', anonymous=False)
        rospy.Subscriber("/robostilt/safety/level", QuaternionStamped, self._level_callback)  

        self.service_stop = rospy.Service('robostilt_state/stop_all_motors', Empty, self.stop_all_motors)
        self.service_set_initial_state = rospy.Service('robostilt_state/set_inital_state', Empty, self.set_initial_state_service)
    ...
    def raise_frame_while_leveling(self,position):
        ...
        while( self.actuators.have_all_actuators_reached_goal()==False):            
            ...
            rospy.loginfo("Roll= " + str(self.roll_x))
    ...
    def set_initial_state(self):
        #service request is empty        
            f.print_ros("Initializing position. All legs raised prismatic to -0.2...")
            f.wait_for_user()              
