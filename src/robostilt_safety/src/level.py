...
pub = rospy.Publisher('/robostilt/safety/level', QuaternionStamped, queue_size=1,latch=True) 
...
def _linkState_callback(data):
    ...
    for i in range (0, len(data.name)):        
        if(data.name[i]=="robostilt::base_link"):
            ...
            (roll_x, pitch_y, yaw_z) = tf.transformations.euler_from_quaternion (orientation_list) 
            #rospy.loginfo("roll_x= %s pitch_y= %s", roll_x , pitch_y)       
    if(abs(roll_x)>max_level_error or abs(pitch_y)> max_level_error):
        rospy.logerr("Level is out of range. Roll_x= " +str(roll_x*rad_to_deg) + " Pitch_y= " +str(pitch_y*rad_to_deg))
